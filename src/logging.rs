    ...
    pub fn peek(&self, len: usize) -> Vec<String> {
        ...
        if let Ok(buffer) = self.buffer.read() {
            ...
            while cursor < buffer.len() && logs.len() < len {
                logs.push(String::from_utf8_lossy(&buffer[cursor]).into_owned());
...
impl std::io::Write for Logger {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        if let (Ok(mut buffer), Ok(mut cursor)) = (self.buffer.write(), self.cursor.write()) {
...
pub fn init(cli: &Cli) -> Result<()> {
    ...
    //                 .expect(&format!("Unable to initialize path for {}", filename));
    //         }
    ...
    // Log file formatting.
    let subscriber = tracing_subscriber::Registry::default().with(cli_logger);
    // .with(file_logger);
